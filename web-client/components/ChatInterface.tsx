...
import { ChatMessage, Document, RagStore, CustomMetadata } from '../types';
...
interface ChatInterfaceProps {
    ...
    onUploadFile: (file: File, metadata: CustomMetadata[]) => void;
    onDeleteFile: (docName: string) => void;
    processingFile: string | null;
    currentStore: RagStore;
}
...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ 
    ...
    return (
        <div className="flex h-screen bg-gem-onyx relative overflow-hidden">
             {/* Sidebar - Collapsible on Mobile, Fixed on Desktop */}
            <aside 
                className={`
                    absolute top-0 bottom-0 left-0 z-20 w-80 bg-gem-onyx border-r border-gem-mist/20 
            ...
            >
                <div className="h-full p-4 overflow-hidden flex flex-col">
                     <div className="flex justify-between items-center lg:hidden mb-4">
                        ...
                        <button onClick={() => setIsSidebarOpen(false)} className="p-2 text-gem-offwhite/60 hover:text-white">
                            ✕
                        </button>
                    </div>
                    <DocumentList 
            ...
            <div className="flex-1 flex flex-col h-full relative min-w-0">
                <header className="absolute top-0 left-0 right-0 p-4 bg-gem-onyx/80 backdrop-blur-sm z-10 flex justify-between items-center border-b border-gem-mist/20">
                    <div className="w-full max-w-4xl mx-auto flex justify-between items-center px-4">
                         <div className="flex items-center min-w-0 flex-1 mr-4">
                            {/* Hamburger Menu */}
                            <button 
                                onClick={() => setIsSidebarOpen(!isSidebarOpen)}
                                className="lg:hidden mr-3 p-2 text-gem-offwhite hover:bg-gem-mist/10 rounded-md flex-shrink-0"
                            >
                             ...
                             <button 
                                onClick={onNewChat}
                                className="mr-3 p-2 text-gem-offwhite hover:text-gem-blue hover:bg-gem-mist/10 rounded-full transition-colors flex-shrink-0"
                                title="Back to Knowledge Bases"
                            >
                                <ArrowLeftIcon />
                            </button>

                            <h1 className="text-xl sm:text-2xl font-bold text-gem-offwhite truncate" title={`Chat with ${documentName}`}>Chat with {documentName}</h1>
                        </div>
                        <button
                            onClick={onNewChat}
